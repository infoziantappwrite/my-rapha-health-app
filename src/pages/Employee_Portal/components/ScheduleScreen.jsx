...
const options = [
  ...
  },
  {
    ...
    color: "text-red-500",
    icon: <TestTubes className="w-6 h-6 text-red-500" />,
    actions: [
  ...
  return (
    <section className="bg-gradient-to-br from-blue-500 to-blue-600 text-white rounded-2xl shadow-md p-8">
      ...
      <div className="text-center mb-8">
        ...
        <button className="mt-5 bg-white text-blue-700 font-medium px-6 py-3 rounded-lg shadow hover:bg-blue-50 flex items-center gap-2 mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
        {options.map((opt) => (
          <div
            key={opt.id}
            className="bg-white text-gray-800 rounded-xl p-5 shadow-sm"
          >
            ...
            <div className="flex flex-col gap-2">
              {opt.actions.map((action, idx) => (
                <button
                  key={idx}
                  className="flex items-center justify-between  border  border-gray-300 rounded-lg px-3 py-2 hover:bg-gray-50 text-sm font-medium"
                >
                  {action.label}
                  {action.icon}
                </button>
      ...
      {/* Footer */}
      <div className="flex flex-col md:flex-row justify-center gap-6 text-sm text-blue-100 border-t border-blue-400 pt-4">
