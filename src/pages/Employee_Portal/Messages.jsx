...
export default function Messages() {
  ...
  return (
    <div className="p-10 space-y-10">
      {/* Header */}
      <h2 className="text-2xl font-bold text-gray-800 mb-1">Messages & Support</h2>
      ...
      <div className="bg-gradient-to-r from-purple-50 to-pink-50 border border-purple-200 rounded-xl p-6 mb-6">
        <div className="flex items-start gap-4">
          ...
          <div>
            ...
            <p className="text-gray-700 font-medium">Dr. Sarah Chen</p>
            <p className="text-purple-600 font-medium">Senior Health Navigator</p>
            ...
            <button
              ...
              className="mt-3 px-5 py-2 bg-purple-600 text-white rounded-lg font-medium hover:bg-purple-700 transition"
            >
              Start Conversation
            </button>
          </div>
      ...
      <div className="bg-white border border-gray-300 rounded-xl p-5">
        <h3 className="text-md font-semibold text-gray-700 mb-4">Recent Messages</h3>
        <div className="space-y-4">
          {messages.map((msg, idx) => (
            <div
              key={idx}
              className={`p-4 rounded-lg border ${
                msg.highlight ? "bg-blue-50 border-blue-200" : "bg-gray-50 border-gray-200"
              }`}
            >
              ...
              <p className="text-gray-600 text-sm mb-3">{msg.body}</p>
              <div className="flex gap-2">
                {msg.actions.map((action, i) => (
                  <button
                    key={i}
                    className="px-3 py-1.5 rounded-md border text-sm font-medium bg-white text-gray-700 hover:bg-gray-100"
                  >
                    {action}
                  </button>
                ))}
      ...
      {chatOpen && (
        <div className="fixed inset-0 bg-black/40 flex justify-center items-center z-50">
          <div className="bg-white w-[400px] rounded-lg shadow-lg flex flex-col max-h-[90vh]">
            {/* Header */}
            <div className="p-4 border-b flex justify-between items-center">
              <div>
                <h3 className="font-semibold text-gray-800">Navigator Chat</h3>
                <p className="text-sm text-gray-500">
                  Connect with Dr. Sarah Chen - <span className="text-green-600">Available now</span>
                </p>
              </div>
              <button
            ...
            <div className="p-4 flex-1 overflow-y-auto">
              <div className="flex items-start gap-2 mb-3">
                <div className="bg-purple-600 text-white w-8 h-8 flex items-center justify-center rounded-full text-sm">
                  SC
                </div>
                <div className="bg-gray-100 p-3 rounded-lg text-sm text-gray-700 max-w-[80%]">
            ...
            {/* Footer */}
            <div className="p-3 text-xs text-gray-400 flex justify-between items-center border-t">
              <button
                onClick={() => setChatOpen(false)}
